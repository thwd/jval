...
func Regex(r *regexp.Regexp) Validator {
	return And(String, Lambda(func(v *jsem.Value, f string) []Error {
		...
		return []Error{
			Error{"value_must_match_regex", f, map[string]string{"regex": r.String()}},
		}
	}))
}
...
// func Relabel(l string, a Validator) Validator {
// 	return Lambda(func(v *jsem.Value, f string) []Error {
// 		es := a.Validate(v, f)
// 		for i, e := range es {
// 			e
...
func LengthBetween(x, y int) Validator {
	...
	return And(Or(String, Array(Anything)), Lambda(func(v *jsem.Value, f string) []Error {
		...
		if l < x || l > y {
			return []Error{
				Error{"value_must_have_length_between", f, map[string]int{"min": x, "max": y}},
			}
		}
		return noErrors
	}))
...
func NumberBetween(x, y float64) Validator {
	...
	return And(Number, Lambda(func(v *jsem.Value, f string) []Error {
		...
		if l < x || l > y {
			return []Error{
				Error{"value_must_have_value_between", f, map[string]float64{"min": x, "max": y}},
			}
		}
		return noErrors
	}))
